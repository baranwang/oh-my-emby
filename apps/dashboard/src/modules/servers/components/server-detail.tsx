  ...
  return (
    <div className="space-y-2 rounded-lg border p-4">
      <div className="flex items-center justify-between gap-4">
        ...
        <span className="bg-muted rounded-md px-2 py-1 text-xs">{healthLabel(health.health)}</span>
      </div>
      <p className="text-muted-foreground text-sm">{freshness}</p>
      {health.lastSuccessAtMs !== null && (
...
const SourceLibraries = ({ sources }: { readonly sources: ReadonlyArray<SourceLibraryView> }) => (
  <section className="space-y-3" aria-labelledby="source-libraries-title">
    <h2 id="source-libraries-title" className="font-heading text-xl font-medium">
      {m.source_libraries()}
    </h2>
    {sources.length === 0 ? (
...
export const ServerDetailPage = ({
  ...
  if (server.isPending) {
    return (
      <div aria-label={m.loading()} className="space-y-4">
  ...
  if (server.isError || !server.data) {
    if (isNotFound(server.error)) {
      return (
        <div role="alert" className="border-destructive/40 space-y-4 rounded-lg border p-4">
          <div>
            <h2 className="text-destructive font-medium">{m.server_not_found_title()}</h2>
            <p className="text-muted-foreground mt-1 text-sm">{m.server_not_found_description()}</p>
          </div>
          {onClose && (
            <Button variant="outline" onClick={onClose}>
              {m.server_back_to_list()}
            </Button>
          )}
        </div>
      );
    ...
    return (
      <div role="alert" className="border-destructive/40 space-y-3 rounded-lg border p-4">
        ...
        <Button variant="outline" onClick={() => void server.refetch()}>
          {m.retry()}
        </Button>
      </div>
    );
