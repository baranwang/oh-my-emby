...
export const LibraryForm = ({ library, groups, onSave, onCancel }: LibraryFormProps) => {
  ...
  const defaultValues: typeof VirtualLibraryInputSchema.Encoded = {
    ...
    sources: library?.sources.map(({ serverId, sourceLibraryId, enabled }) => ({ serverId, sourceLibraryId, enabled })) ?? [],
        ...
        {(field) => (
          <div className="space-y-2">
            ...
            {field.state.meta.errors.length > 0 && (
              <p id={`${field.name}-error`} className="text-sm text-destructive">{m.library_name_required()}</p>
            )}
          </div>
        )}
      </form.Field>
              ...
              {(mediaType) => (
                <SourceBindings
                  ...
                  onToggle={(serverId, sourceLibraryId, enabled) => {
                    const index = field.state.value.findIndex((item) =>
                    ...
                    field.handleChange(index === -1
                      ...
                      : field.state.value.map((item, itemIndex) => itemIndex === index ? { ...item, enabled } : item))
                  }}
                />
              )}
            </form.Subscribe>
