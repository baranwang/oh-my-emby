...
const fetchFor = (options?: {
  ...
  vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (pathname === "/api/dashboard/session")
      return json({ authenticated: true, username: "owner" });
    ...
    if (pathname === "/api/dashboard/servers/server-1/test") {
...
describe("route-controlled server Drawer", () => {
  it.each(["/dashboard/servers?new=true", "/dashboard/servers/server-1"])(
    "keeps form actions in a footer outside the scrolling body at %s",
    async (path) => {
      await renderRoute(path, fetchFor());
      await vi.waitFor(() =>
      ...
      const scrollArea = content.querySelector('[data-slot="scroll-area"]')!;
      const footer = content.querySelector('[data-slot="drawer-footer"]');
      expect(footer).not.toBeNull();
      expect(footer!.parentElement).toBe(content);
      expect(scrollArea.contains(footer)).toBe(false);
      expect(button(m.cancel()).closest('[data-slot="drawer-footer"]')).toBe(footer);
      expect(button(m.save()).closest('[data-slot="drawer-footer"]')).toBe(footer);
      expect(button(m.save()).form).toBe(content.querySelector("form"));
    },
  );

  it("puts the validation summary in the footer while keeping field errors by their inputs", async () => {
    await renderRoute("/dashboard/servers?new=true", fetchFor());
    await act(async () => button(m.save()).click());
    await vi.waitFor(() =>
      expect(document.body.textContent).toContain(m.server_validation_failed()),
    );

    const summary = document.getElementById("server-form-error")!;
    const password = document.querySelector<HTMLInputElement>('input[name="upstream-password"]')!;
    const fieldError = document.getElementById(password.getAttribute("aria-describedby") ?? "");
    expect(summary.closest('[data-slot="drawer-footer"]')).not.toBeNull();
    expect(summary.closest('[data-slot="scroll-area"]')).toBeNull();
    expect(document.activeElement).toBe(summary);
    expect(password.form?.getAttribute("aria-describedby")).toBe(summary.id);
    expect(password.getAttribute("aria-invalid")).toBe("true");
    expect(fieldError).not.toBeNull();
    expect(fieldError!.closest('[data-slot="scroll-area"]')).not.toBeNull();
  });
  ...
  it("opens create from ?new=true and close returns to the collection route", async () => {
    ...
    await act(async () => {
      await vi.waitFor(() => expect(router.state.location.publicHref).toBe("/dashboard/servers"));
    });
  });
  ...
  it("closes after a successful save and retains the Drawer plus values after failure", async () => {
    ...
    const username = document.body.querySelector(
      'input[name="upstream-username"]',
    ) as HTMLInputElement;
    const password = document.body.querySelector(
      'input[name="upstream-password"]',
    ) as HTMLInputElement;
    await act(async () => {
    ...
    await act(async () => {
      await vi.waitFor(() =>
        expect(success.router.state.location.publicHref).toBe("/dashboard/servers"),
      );
    });
    ...
    const failedHost = document.body.querySelector(
      'input[name="endpoints[0].host"]',
    ) as HTMLInputElement;
    const failedUsername = document.body.querySelector(
      'input[name="upstream-username"]',
    ) as HTMLInputElement;
    const failedPassword = document.body.querySelector(
      'input[name="upstream-password"]',
    ) as HTMLInputElement;
    await act(async () => {
      for (const [input, value] of [
    ...
    await act(async () => {
      await vi.waitFor(() => expect(document.body.textContent).toContain(m.server_save_failed()));
    });
    ...
    expect(summary.closest('[data-slot="drawer-footer"]')).not.toBeNull();
    expect(document.activeElement).toBe(summary);
    expect(button(m.save()).disabled).toBe(false);
  });

  it("keeps a saved server open after discovery failure and retries with an update instead of a duplicate create", async () => {
    const options = { discoveryFails: true };
    const fetch = fetchFor(options);
    const { router } = await renderRoute("/dashboard/servers?new=true", fetch);
    await act(async () => {
      for (const [name, value] of [
        ["name", "New server"],
        ["endpoints[0].host", "emby.example.com"],
    ...
    await act(async () => {
      await vi.waitFor(() =>
        expect(document.getElementById("server-form-error")?.textContent).toContain(
    ...
    expect(router.state.location.publicHref).toBe("/dashboard/servers?new=true");
    expect(
      document.getElementById("server-form-error")?.closest('[data-slot="drawer-footer"]'),
    ).not.toBeNull();
    expect(
      document.body.querySelector<HTMLInputElement>('input[name="upstream-password"]')?.value,
    ).toBe("secret");
    options.discoveryFails = false;
    await act(async () => button(m.save()).click());
    await act(async () => {
      await vi.waitFor(() => expect(router.state.location.publicHref).toBe("/dashboard/servers"));
    });
    ...
    expect(
      requests.filter(
        (request) =>
          request.method === "POST" && new URL(request.url).pathname === "/api/dashboard/servers",
      ),
    ).toHaveLength(1);
    expect(
      requests.filter(
        (request) =>
          request.method === "PUT" &&
          new URL(request.url).pathname === "/api/dashboard/servers/server-1",
      ),
    ).toHaveLength(1);
    expect(
      requests.some(
        (request) => new URL(request.url).pathname === "/api/dashboard/servers/server-1/libraries",
      ),
    ).toBe(true);
  });
});
