...
describe("server query behavior", () => {
  it.each(["create", "update"])(
    "%s automatically connects and fetches fresh source libraries after saving",
    async (operation) => {
      const requests: Array<string> = [];
      vi.stubGlobal("fetch", async (input: RequestInfo | URL, init?: RequestInit) => {
        const request = input instanceof Request ? input : new Request(input, init);
        requests.push(`${request.method} ${new URL(request.url).pathname}`);
        return savedServerFetch(request);
      });
      const queryClient = new QueryClient({
        defaultOptions: { queries: { retry: false, staleTime: Infinity } },
      });
      queryClient.setQueryData(queryKeys.serverLibraries("server-1"), [
        { ...sources[0], name: "Old name" },
      ]);

      const saved =
        operation === "create"
          ? await createServer(input, queryClient)
      ...
      expect(requests).toEqual([
        operation === "create"
          ...
          : "PUT /api/dashboard/servers/server-1",
        "POST /api/dashboard/servers/server-1/test",
        "GET /api/dashboard/servers/server-1/libraries",
      ]);
      expect(queryClient.getQueryData(queryKeys.serverLibraries("server-1"))).toEqual(sources);
    },
  );

  it.each(["connection", "libraries"])(
    "distinguishes a saved configuration from a %s discovery failure",
    async (stage) => {
      vi.stubGlobal("fetch", async (request: RequestInfo | URL) => {
        const pathname = new URL(request instanceof Request ? request.url : request.toString())
          .pathname;
        if (pathname.endsWith(stage === "connection" ? "/test" : "/libraries")) {
          return json(
            { _tag: "UpstreamUnavailable", serverId: "server-1", detail: "upstream offline" },
            503,
          );
        }
        return savedServerFetch(request);
      ...
      await expect(createServer(input, queryClient)).rejects.toMatchObject({
        server: { id: "server-1" },
        stage,
        cause: { _tag: "UpstreamUnavailable", detail: "upstream offline" },
      });
      expect(queryClient.getQueryState(queryKeys.servers)?.isInvalidated).toBe(true);
    },
  );

  it("does not fetch libraries when the connection response has no reachable endpoints", async () => {
    const requests: Array<string> = [];
    vi.stubGlobal("fetch", async (request: RequestInfo | URL) => {
      const pathname = new URL(request instanceof Request ? request.url : request.toString())
        .pathname;
      ...
      return pathname.endsWith("/test")
        ? json({ reachable: false, catalogId: null, endpoints: [] })
        : savedServerFetch(request);
    });
    const queryClient = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    ...
    expect(requests).toEqual(["/api/dashboard/servers", "/api/dashboard/servers/server-1/test"]);
  });

  it("saves disabled servers without connecting or discovering libraries", async () => {
    const requests: Array<string> = [];
    vi.stubGlobal("fetch", async (request: RequestInfo | URL) => {
      requests.push(
        new URL(request instanceof Request ? request.url : request.toString()).pathname,
      );
      return json({ ...server, enabled: false });
    });
    const queryClient = new QueryClient();

    expect((await createServer({ ...input, enabled: false }, queryClient)).enabled).toBe(false);
    expect(requests).toEqual(["/api/dashboard/servers"]);
  });
  ...
  it("invalidates the saved server and every existing Overview constituent without optimistic writes", async () => {
    vi.stubGlobal("fetch", savedServerFetch);
    const queryClient = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    ...
    expect(invalidate).toHaveBeenCalledWith({
      queryKey: queryKeys.server("server-1"),
      exact: true,
    });
    expect(invalidate).toHaveBeenCalledWith({ queryKey: queryKeys.servers, exact: true });
    expect(invalidate).toHaveBeenCalledWith({
      queryKey: queryKeys.serverHealth("server-1"),
      exact: true,
    });
    expect(invalidate).toHaveBeenCalledWith({
      queryKey: queryKeys.serverLibraries("server-1"),
      exact: true,
    });
  ...
  it("re-evaluates health polling when page visibility changes", async () => {
    ...
    const fetch = vi.fn(async () =>
      json({ serverId: "server-1", health: "healthy", lastSuccessAtMs: 1 }),
    );
  ...
  it("discovers source libraries through the dedicated server query", async () => {
    ...
    const sources = await queryClient.fetchQuery(
      serverLibrariesQueryOptions("server-1", queryClient),
    );
    ...
    providers: [
      {
        ...
        order: 0,
        language: "en-US",
        credential: { _tag: "Set", value: "token" },
      },
