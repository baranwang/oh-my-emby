...
describe("route-controlled virtual-library Drawer", () => {
  ...
  it("closes after save and keeps the create Drawer plus values after failure", async () => {
    ...
    const source = document.body.querySelector('fieldset [role="switch"]') as HTMLElement;
    await act(async () => {
    ...
    await act(async () => {
      await vi.waitFor(() =>
        expect(success.router.state.location.publicHref).toBe("/dashboard/libraries"),
      );
    });
    ...
    const failed = await renderRoute(
      "/dashboard/libraries?new=true",
      fetchFor({ saveFails: true }),
    );
    await vi.waitFor(() =>
      expect(document.body.querySelector('fieldset [role="switch"]')).toBeInstanceOf(HTMLElement),
    );
    const failedName = document.body.querySelector('input[name="name"]') as HTMLInputElement;
    const failedSource = document.body.querySelector('fieldset [role="switch"]') as HTMLElement;
    await act(async () => {
    ...
    await act(async () => {
      await vi.waitFor(() => expect(document.body.textContent).toContain(m.library_save_failed()));
    });
