...
vi.mock("@tanstack/react-router", async (importOriginal) => {
  ...
  return {
    ...
    Link: ({
      ...
      <a {...props} href={to.replace(/\$(\w+)/g, (_, key: string) => params?.[key] ?? `$${key}`)}>
        {children}
      </a>
    ),
    useNavigate: () => vi.fn(),
...
import {
  ServerDetailPage,
  ServerHealthStatus,
} from "../src/modules/servers/components/server-detail.js";
...
const renderOverview = (queryClient: QueryClient) =>
  render(
...
describe("authenticated shell", () => {
  it("renders shell content with exactly one main landmark and localized navigation controls", async () => {
    vi.stubGlobal(
      "matchMedia",
      vi.fn(() => ({
        matches: false,
    ...
    expect(view.container.querySelector("#main-content")?.textContent).toContain("Dashboard");
    expect(
      view.container.querySelector('[data-sidebar="trigger"]')?.getAttribute("aria-label"),
    ).toBe(m.navigation_toggle());
    expect(view.container.querySelector('[data-sidebar="rail"]')?.getAttribute("aria-label")).toBe(
      m.navigation_toggle(),
    );
  });
  ...
  it("closes the mobile drawer after selecting navigation", async () => {
    ...
    const trigger = view.container.querySelector<HTMLButtonElement>('[data-sidebar="trigger"]')!;
    await act(async () => trigger.click());
    const dialog = document.querySelector('[role="dialog"]');
    expect(dialog).not.toBeNull();
    const link = [...dialog!.querySelectorAll("a")].find(
      (candidate) => candidate.textContent === m.servers(),
    )!;
...
describe("Overview states", () => {
  ...
  it("renders one actionable error state", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => json({ _tag: "Internal", requestId: "request-1" }, 500)),
    );
    const view = await renderOverview(overviewClient());
    await act(async () => {
      await vi.waitFor(() =>
        expect(view.container.textContent).toContain("Overview could not be loaded."),
      );
    });
  ...
  it("renders the next setup action when no server exists", async () => {
    const view = await renderOverview(
      overviewClient({ servers: [], libraries: [], system, failures: [] }),
    );
...
describe("server page states", () => {
  ...
  it.each([
    ...
    "includes the visible label and server name in each edit link in %s",
    async (locale, label) => {
...
it("distinguishes missing health from stale health", () => {
  const missing = renderToStaticMarkup(
    <ServerHealthStatus
      health={{ serverId: server.id, health: "unknown", lastSuccessAtMs: null }}
      nowMs={120_000}
    />,
  );
  const stale = renderToStaticMarkup(
    <ServerHealthStatus
      health={{ serverId: server.id, health: "degraded", lastSuccessAtMs: 1 }}
      nowMs={120_000}
    />,
  );
...
describe("library create prerequisites", () => {
  ...
  it("shows server loading without silently rendering an empty form", async () => {
    ...
    expect(document.body.querySelector('[data-slot="skeleton"]')?.getAttribute("aria-label")).toBe(
      m.servers_loading(),
    );
  ...
  it("discovers libraries for a healthy enabled server without a stable catalog id", async () => {
    ...
    await act(async () => {
      ...
      await vi.waitFor(() =>
        expect(document.body.querySelector('fieldset [role="switch"]')).toBeInstanceOf(HTMLElement),
      );
    });
  });
});
...
it("keeps configured bindings visible and disable-able during discovery errors", async () => {
  ...
  const sourceSwitch = view.container.querySelector<HTMLElement>('fieldset [role="switch"]');
  expect(sourceSwitch).toBeInstanceOf(HTMLElement);
  expect(sourceSwitch?.getAttribute("aria-checked")).toBe("true");

  await act(async () => sourceSwitch!.click());
  expect(sourceSwitch?.getAttribute("aria-checked")).toBe("false");
  expect(view.container.textContent).toContain(m.source_binding_disabled());
});

it("saves the library enabled switch independently of its source switches", async () => {
  const onSave = vi.fn();
  const view = await render(
    <LibraryForm
      library={library}
  ...
  const enabledLabel = view.container.querySelector<HTMLLabelElement>('label[for="enabled"]');
  expect(enabledLabel?.textContent).toBe(m.library_enabled());
  await act(async () => enabledLabel!.click());
  ...
  expect(onSave).toHaveBeenCalledExactlyOnceWith({
    ...
    sources: [{ serverId: server.id, sourceLibraryId: source.id, enabled: true }],
    enabled: false,
  });
});
...
it("associates the required-source error with its fieldset", async () => {
  const view = await render(
    <LibraryForm groups={[{ server, state: "success", sources: [source] }]} onSave={vi.fn()} />,
  );
  ...
  expect(view.container.querySelector(`#${descriptionId}`)?.textContent).toBe(
