...
const upstreamItem = (serverId: string) => ({
  ...
  Studios: [{ Name: "Warner", Id: `${serverId}-studio` }],
  MediaSources: [
    {
      Id: `${serverId}-media`,
...
export const acceptanceUpstreamFetch: typeof fetch = async (input, init) => {
  ...
  if (
    ["setup.example.com", "primary.example.com", "secondary.example.com"].includes(url.hostname) &&
    url.pathname.endsWith("/Users/AuthenticateByName") &&
    request.method === "POST"
  ) {
  ...
  if (source !== undefined && url.pathname === "/Studios") {
    if (
      url.searchParams.get("UserId") !== `upstream-user-${source}` ||
      url.searchParams.get("ParentId") !== `movies-${source}`
    ) {
    ...
    return source === "2"
      ...
      : Response.json({
          Items: [{ Name: "Warner", Id: `private-studio-${source}` }],
          TotalRecordCount: 1,
        });
  }
  if (source !== undefined && url.pathname === "/Items") {
    const serverId = `server-${source}`;
    // Real Emby rejects user-dependent sorting without the upstream user's ID.
    if (
      url.searchParams.get("SortBy")?.includes("IsFavoriteOrLiked") &&
      url.searchParams.get("UserId") !== `upstream-user-${source}`
    ) {
      return new Response("Missing upstream user for favorites sorting", { status: 500 });
    }
...
export const crossPlatformAcceptance = (name: "workers" | "docker", harness: AcceptanceHarness) =>
  describe(`${name} cross-platform acceptance`, () => {
    ...
    it("aligns ordered endpoints, User-Agent policy, and metadata fallback without exposing secrets", async () => {
      ...
      const requestItems = (clientUserAgent?: string) =>
        Effect.runPromise(
          Effect.gen(function* () {
          ...
          providers: [
            {
              ...
              order: 0,
              language: "en-US",
              credential: { _tag: "Set", value: secretValues[1] },
            },
            {
              id: "trakt",
              enabled: true,
              order: 1,
              language: null,
              credential: { _tag: "Set", value: secretValues[2] },
            },
          ],
        }),
      });
      ...
      expect(await savedSettings.clone().json()).toEqual({
        providers: [
          {
            id: "tmdb",
            enabled: true,
      ...
      expect(await reorderedSettings.clone().json()).toEqual({
        providers: [
          {
            id: "trakt",
            enabled: true,
      ...
      const publicSettings = await app.request("/api/dashboard/metadata-settings", {
        headers: { cookie },
      });
      ...
      await expect(publicSettings.clone().json()).resolves.toEqual({
        providers: [
          {
            id: "trakt",
            enabled: true,
          ...
          providers: [
            {
              ...
              order: 0,
              language: null,
              credential: { _tag: "Set", value: secret },
            },
            {
              id: "trakt",
              enabled: false,
    ...
    it("preserves repository encoding, source order, and applied migrations", async () => {
      await useRepositories(app.repositories, (repositories) =>
        Effect.gen(function* () {
          ...
          yield* repositories.saveVirtualLibrary(
            {
              ...
              sources: [
                {
                  serverId: "server-1" as any,
                  sourceLibraryId: "movies-1" as any,
                  sourceLibraryName: "One",
                  mediaType: "movies",
                  sourceOrder: 1,
                  enabled: true,
                },
                {
                  serverId: "server-0" as any,
                  sourceLibraryId: "movies-0" as any,
                  sourceLibraryName: "Zero",
                  mediaType: "movies",
                  sourceOrder: 0,
                  enabled: true,
                },
          ...
          expect(
            (yield* repositories.resolveEligibleSources("library-1")).map(
              ({ serverId }) => serverId,
            ),
    ...
    it("serves the Rex home, library, resume, history, and studio HTTP sequence", async () => {
      await claimOwner(app);
      await prepareFederation(app);
      const token = await loginEmby(app);
      const headers = { "x-emby-token": token, "user-agent": "Rex-Standard/0.1.0" };
      const get = async (path: string) => {
        const response = await app.request(`/emby${path}`, { headers });
        expect(response.status, path).toBe(200);
        const text = await response.text();
        expect(text).not.toMatch(/upstream-password|token-\d|private-studio|untrusted/);
      ...
      const views = await get("/Users/owner/Views");
      expect(views.Items).toMatchObject([{ Id: "library-1", Type: "CollectionFolder" }]);
      const home = await get(
        "/Users/owner/Items?IncludeItemTypes=Series,Movie,Video,MusicVideo&Limit=30&Recursive=true&SortBy=IsFavoriteOrLiked,Random&SortOrder=Descending&StartIndex=0",
      );
      expect(home.Items).toHaveLength(1);
      const id = home.Items[0]!.Id;
      const library = await get("/Users/owner/Items?ParentId=library-1&Limit=100");
      expect(library.Items.map(({ Id }) => Id)).toEqual([id]);
      const detail = await app.request(`/emby/Users/owner/Items/${encodeURIComponent(id)}`, {
        headers,
      });
      expect(detail.status).toBe(200);
      await expect(detail.json()).resolves.toMatchObject({ Id: id, Type: "Movie" });

      const write = async (patch: Record<string, unknown>) => {
        const response = await app.request(
          `/emby/Users/owner/Items/${encodeURIComponent(id)}/UserData`,
          {
            method: "POST",
      ...
      await write({ PlaybackPositionTicks: 12345, Played: false });
      const resume = await get(
        "/Users/owner/Items/Resume?Limit=99&MediaTypes=Video&Recursive=true",
      );
      expect(resume.Items).toMatchObject([
        { Id: id, UserData: { Played: false, PlaybackPositionTicks: 12345 } },
      ]);
      await write({ Played: true, PlaybackPositionTicks: 0 });
      expect((await get("/Users/owner/Items/Resume")).Items).toEqual([]);
      expect(
        (await get("/Users/owner/Items?Filters=IsPlayed&SortBy=DatePlayed&SortOrder=Descending"))
          .Items,
      ).toMatchObject([{ Id: id, UserData: { Played: true } }]);
      const studios = await get("/Studios?Limit=10000");
      expect(studios).toMatchObject({
        Items: [{ Id: "studio:warner", Name: "Warner", Type: "Studio" }],
        TotalRecordCount: 1,
      });
      expect((await get("/Items?StudioIds=studio%3Awarner")).Items.map(({ Id }) => Id)).toEqual([
        id,
      ]);
      const unauthorized = await app.request("/emby/Studios");
      expect(unauthorized.status).toBe(401);
      const forbidden = await app.request("/emby/Users/owner/Items/Resume?UserId=another-user", {
        headers,
      });
    ...
    it("recovers persisted outbox targets after reopening repositories", async () => {
      const scenario = await prepareFederation(app);
      await useRepositories(app.repositories, (repositories) =>
        repositories.writeUserStateAndTargets({
    ...
    it.skipIf(name !== "docker")(
      "redirects image URLs from the Bun production runtime",
      async () => {
        await claimOwner(app);
        const accessToken = await loginEmby(app);
        const scenario = await prepareFederation(app);
        ...
        for (const prefix of ["", "/emby"]) {
          const image = await app.request(
            `${prefix}/Items/${scenario.page.items[0]!.id}/Images/Primary`,
            {
              headers: { authorization: `Bearer ${accessToken}` },
              redirect: "manual",
            },
