...
const benchmarkScript = resolve(
  import.meta.dirname,
  "../../../scripts/benchmark-workers-pbkdf2.ts",
);
  ...
  const server = Bun.serve({
    ...
    fetch: async (request) => {
      ...
      if (
        ...
        request.headers.get("authorization") !== `Bearer ${apiToken}`
      ) {
      ...
      if (
        request.method === "GET" &&
        url.pathname.startsWith(`/client/v4/accounts/${accountId}/workers/scripts/`)
      ) {
        if (scenario === "existing")
          return Response.json({ success: true, result: { id: "pre-existing" } });
        ...
        if (scenario === "duplicate-not-found") {
          return Response.json(
            { success: false, errors: [{ code: 10007 }, { code: 10007 }] },
            { status: 404 },
          );
        }
        if (scenario === "empty-not-found")
          return Response.json({ success: false, errors: [] }, { status: 404 });
        if (scenario === "malformed-not-found")
          return Response.json({ success: false, errors: {} }, { status: 404 });
        return Response.json(
          { success: false, errors: [{ code: 10007, message: "script not found" }], result: null },
          { status: 404 },
        );
      }
      if (
      ...
      if (
        request.method === "DELETE" &&
        url.pathname === `/client/v4/accounts/${accountId}/d1/database/${databaseId}`
      ) {
        return Response.json({ success: true, errors: [], messages: [], result: null });
      }
      ...
      if (request.method === "GET" && url.pathname === "/dashboard/servers") {
        return new Response("<!doctype html><title>Dashboard</title>", {
      ...
      if (request.method === "POST" && url.pathname === "/api/dashboard/claim") {
        return Response.json(
          { authenticated: true },
          { headers: { "set-cookie": "ome_session=fake; Path=/api/dashboard; HttpOnly" } },
        );
      }
...
const workerResource = (requests: ReadonlyArray<string>) => {
  const request = requests.find((value) =>
    value.startsWith(`GET /client/v4/accounts/${accountId}/workers/scripts/`),
  );
  expect(request).toBeDefined();
  return request!.slice(request!.lastIndexOf("/") + 1);
};

const workerDeletes = (requests: ReadonlyArray<string>) =>
  requests.filter((value) =>
    value.startsWith(`DELETE /client/v4/accounts/${accountId}/workers/scripts/`),
  );

const d1Deletes = (requests: ReadonlyArray<string>) =>
  requests.filter(
    (value) => value === `DELETE /client/v4/accounts/${accountId}/d1/database/${databaseId}`,
  );
...
describe.sequential("Workers remote smoke ownership", () => {
  it.each([
    ...
    "aborts on %s preflight without deploy or cleanup",
    async (scenario) => {
  ...
  it.each(["missing-ownership", "wrong-ownership"] as const)(
    "does not delete a Worker after a %s response",
    async (scenario) => {
      const result = await runRemoteScenario(scenario);
      const worker = workerResource(result.requests);
