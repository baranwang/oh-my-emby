...
const runBudgetScenario = async (): Promise<BudgetEvidence> => {
  ...
  try {
    ...
    const measuredRepositories = Layer.effect(
      Repositories,
      Effect.gen(function* () {
        const repositories = yield* Repositories;
    ...
    const response = (
      serverId: string,
      path: string,
    ): Effect.Effect<unknown, UpstreamUnavailable> => {
    ...
    const upstream = Layer.succeed(
      UpstreamClient,
      UpstreamClient.of({
        request: ({ serverId, path }, schema) =>
          response(serverId, path).pipe(
    ...
    const materializedRows = inspection
      .query<{ count: number }, []>("SELECT COUNT(*) AS count FROM query_generation_items")
      .get()!.count;
    inspection.close();
