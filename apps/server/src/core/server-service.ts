...
export interface ServerServiceApi {
  readonly list: () => Effect.Effect<
    ReadonlyArray<ServerView>,
    import("./errors.js").RepositoryError
  >;
  readonly get: (
    serverId: string,
  ) => Effect.Effect<ServerView, ServerNotFound | import("./errors.js").RepositoryError>;
  readonly getRecord: (
    serverId: string,
...
const normalizedEndpoint = (
  ...
  const existing =
    byId ??
    current.find((endpoint) => normalizeUpstreamBaseUrl(endpointUrl(endpoint).href) === displayUrl);
  const unchanged =
    existing !== undefined && normalizeUpstreamBaseUrl(endpointUrl(existing).href) === displayUrl;
  return {
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getRecord: ServerServiceApi["getRecord"] = (serverId) =>
      repositories
        .getServer(serverId)
        .pipe(
    ...
    const list: ServerServiceApi["list"] = () =>
      repositories.listServers().pipe(Effect.map((servers) => servers.map(toView)));

    const get: ServerServiceApi["get"] = (serverId) => getRecord(serverId).pipe(Effect.map(toView));

    const resolveName = (server: UpstreamServer) =>
      Effect.gen(function* () {
        if (server.name !== "") return server.name;
        for (const endpoint of server.endpoints) {
          const result = yield* upstream.probeServerName(server, endpoint).pipe(Effect.result);
          if (result._tag === "Success" && result.success !== null) return result.success;
        }
        return "MyEmby";
      });

    const create: ServerServiceApi["create"] = (input) =>
      Effect.gen(function* () {
        ...
        const name = yield* resolveName(candidate);
    ...
    const update: ServerServiceApi["update"] = (serverId, input) =>
      Effect.gen(function* () {
        ...
        if (current.verifiedCatalogId !== null || current.verifiedBaseUrl !== null) {
          ...
          for (const endpoint of changedEndpoints) {
            const probed = yield* upstream
              .probeServerIdentity(candidate, endpoint)
              .pipe(Effect.result);
            if (probed._tag === "Failure") {
              if (
                probed.failure._tag === "UpstreamUnavailable" ||
                probed.failure._tag === "UpstreamTimeout" ||
                (probed.failure._tag === "UpstreamRejected" &&
                  [500, 502, 503, 504].includes(probed.failure.status))
              )
                continue;
    ...
    const remove: ServerServiceApi["delete"] = (serverId) =>
      Effect.gen(function* () {
        yield* getRecord(serverId);
        yield* repositories.deleteServer(serverId);
      });

    const beginRequest: ServerServiceApi["beginRequest"] = (serverId) =>
      getRecord(serverId).pipe(
    ...
    const persistResult: ServerServiceApi["persistResult"] = (request, result) => {
      const command: SaveServerResultCommand = {
        ...
        ...(result.accessTokenExpiresAtMs === undefined
          ? {}
          : { accessTokenExpiresAtMs: result.accessTokenExpiresAtMs }),
        ...(result.upstreamUserId === undefined ? {} : { upstreamUserId: result.upstreamUserId }),
        ...(result.verifiedCatalogId === undefined
          ? {}
          : { verifiedCatalogId: result.verifiedCatalogId }),
        ...(result.verifiedBaseUrl === undefined
          ? {}
          : { verifiedBaseUrl: result.verifiedBaseUrl }),
        ...(result.health === undefined ? {} : { health: result.health }),
        ...(result.lastSuccessAtMs === undefined
          ? {}
          : { lastSuccessAtMs: result.lastSuccessAtMs }),
      };
      return repositories
        .saveServerResult(command)
        .pipe(
    ...
    const testConnection: ServerServiceApi["testConnection"] = (
      serverId,
      ...
      Effect.gen(function* () {
        ...
        for (const endpoint of server.endpoints) {
          const attempted = yield* upstream
          ...
          const sameCatalog =
            identity !== null
              ? identity === catalogId
              : catalogId === null &&
                (verifiedBaseUrl === null || verifiedBaseUrl === endpoint.displayUrl);
          if (!sameCatalog) {
            endpoints.push({
              ...endpoint,
              verifiedCatalogId: null,
    ...
    const listSourceLibraries: ServerServiceApi["listSourceLibraries"] = (serverId) =>
      Effect.gen(function* () {
        const server = yield* getRecord(serverId);
        if (
          !server.enabled ||
          ...
          !server.endpoints.some(
            ({ health, verifiedCatalogId }) =>
              health === "healthy" && verifiedCatalogId === server.verifiedCatalogId,
          )
        ) {
